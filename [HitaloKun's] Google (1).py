...
async def _(event):
    ...
    await event.edit("Searched on Google for `{}` in `{}` seconds. \n\n{}".format(input_str, ms, output_str), link_preview=False)
    await asyncio.sleep(3)
    await event.edit("**Google Search:** `{}`\n\n{}".format(input_str, output_str), link_preview=False)
...
async def _(event):
    ...
    ms = (end - start).seconds
    await event.edit("Searched on Google Images for `{}` in `{}` seconds.".format(input_str, ms), link_preview=False)
    await asyncio.sleep(3)
    await event.delete()
